...
module.exports = {
  ...
  RhelenaPresentationModel : function() {
      ...
      this.persist = function(group, id) {
        ...
        if (_self.__persistenceGroup && _self.__persistenceId && _self.__persistenceGroup===group && _self.__persistenceId===id) { //if the model is already persisted, update local properties with global values
            Object.getOwnPropertyNames(_self).forEach(function(propertyName) {
                if (!isPrivateProperty(_self, propertyName) && module.exports.globalState[_self.__persistenceGroup][_self.__persistenceId][propertyName]) {
                    _self[propertyName] = module.exports.globalState[_self.__persistenceGroup][_self.__persistenceId][propertyName]
                }
            })
            return;
        }
